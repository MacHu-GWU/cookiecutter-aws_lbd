...
2. **Clone and enter the project:**
   ...
   .. code-block:: bash
       ...
       git clone <repo-url>
       cd {{ cookiecutter.package_name }}-project
...
:mod:`~{{ cookiecutter.package_name }}.one.one_01_config`).  On Lambda,
...
You edited files in :mod:`~{{ cookiecutter.package_name }}.lbd` or other package
...
- ``{{ cookiecutter.package_name }}/lbd/*.py`` (or any package source code)
...
You edited :mod:`~{{ cookiecutter.package_name }}.cdk.stacks.infra_stack` to add
...
- ``{{ cookiecutter.package_name }}/cdk/stacks/infra_stack.py``

If you added a new ``CfnOutput``, also update
``{{ cookiecutter.package_name }}/cdk/stacks/infra_stack_exports.py`` with a
...
.. list-table::
   ...
   * - File
     - Action
   * - ``{{ cookiecutter.package_name }}/lbd/my_func.py``
     - **Create** — handler module (Input/Output/main)
   * - ``{{ cookiecutter.package_name }}/lambda_function.py``
     - **Edit** — add ``from ... import lambda_handler as my_func_handler``
   * - ``.env.shared``
     - **Edit** — add ``LBD_FUNC_MY_FUNC_*`` env vars
   * - ``{{ cookiecutter.package_name }}/config/config_00_main.py``
     - **Edit** — add ``lbd_func_my_func`` field
   * - ``{{ cookiecutter.package_name }}/one/one_01_config.py``
...
``cd cdk && cdk destroy {{ cookiecutter.package_name | slugify }}-lbd --profile $LOCAL_AWS_PROFILE``.
...
See :mod:`~{{ cookiecutter.package_name }}.cdk.stacks.lambda_stack` line ~88 for
the explanation.
